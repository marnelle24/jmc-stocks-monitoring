<div>
                    ...
                    <td class="p-4 border-y border-gray-300">
                        @if (count($user->roles))
                            @foreach ($user->roles as $role)
                                <span
                                    class="{{ $role->name === 'admin' ? 'bg-green-600' : ($role->name === 'administrator' ? 'bg-red-600' : 'bg-yellow-600') }} text-md rounded-full px-2.5 m-0.5 mx-0.5 inline-flex py-1 drop-shadow text-sm capitalize text-white text-left">
                                    {{$role->name}}
                                </span>
                            @endforeach
                        @endif
    ...
    @livewire('user.role-and-permissions', [], key('role-permission-modal'))
    <br />
    <div class="p-3">
